...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('cupof', function (Blueprint $table) {
            ...
            // Relación con materia que se dicta en esta posición
            $table->foreignId('id_materias')->constrained('materias')->onDelete('cascade');
            // Relación con curso donde se dicta la materia
            $table->foreignId('id_cursos')->constrained('cursos')->onDelete('cascade');
            // Relación con grupo específico del curso
            ...
            $table->string('cargo', 5)->default('PF'); // Tipo de cargo (PF=Profesor de Front, etc.)
            $table->timestamps();
        });
    }
