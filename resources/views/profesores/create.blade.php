        ...
        <form action="{{ route('horarios.store') }}" method="POST">
            ...
            <div>
                ...
                <select name="turno" id="turno" required>
                    ...
                    @foreach ($turnos as $label => $value)
                        <option value="{{ $value }}" {{ old('turno') == $value ? 'selected' : '' }}>
                            {{ $label }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="id_grupos" id="id_grupos" required>
                    <option value="">-- seleccionar --</option>
                    @foreach ($grupos as $g)
                        <option value="{{ $g->id }}" {{ old('id_grupos') == $g->id ? 'selected' : '' }}>
                            {{ $g->nombre }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="id_materias" id="id_materias" required>
                    ...
                    @foreach ($materias as $m)
                        <option value="{{ $m->id }}" {{ old('id_materias') == $m->id ? 'selected' : '' }}>
                            {{ $m->nombre }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="id_salones" id="id_salones" required>
                    ...
                    @foreach ($salones as $s)
                        <option value="{{ $s->id }}" {{ old('id_salones') == $s->id ? 'selected' : '' }}>
                            {{ $s->numero }}</option>
                    @endforeach
                </select>
            </div>
                    ...
                    @foreach ($dias as $key => $label)
                        <label style="margin-right:10px">
                            <input type="checkbox" name="dias[]" value="{{ $key }}"
                                {{ is_array(old('dias')) && in_array($key, old('dias')) ? 'checked' : '' }}>
                            {{ $label }}
                        </label>
                    @endforeach
                </div>
                ...
                <div>
                    @foreach ($horas as $h)
                        <label style="display:inline-block; width:200px; margin:4px">
                            <input type="checkbox" name="horas[]" value="{{ $h->id }}"
                                {{ is_array(old('horas')) && in_array($h->id, old('horas')) ? 'checked' : '' }}>
